/**
 * Shared validation rules for branching-video configs.
 *
 * Works in both Node (CommonJS `require`) and the browser (global
 * `ConfigValidator`). The CLI in validate-config.js handles file reading and
 * JSON parsing, then calls `validate()`. The editor (editor.html) parses the
 * draft in memory and calls the same `validate()` so the rules never drift.
 *
 * `validate(config)` takes an already-parsed config object and returns
 * `{ errors: string[], warnings: string[] }`. It does not read files, parse
 * JSON, or exit the process.
 */
(function (root, factory) {
  if (typeof module === 'object' && module.exports) {
    module.exports = factory();
  } else {
    root.ConfigValidator = factory();
  }
  ...
  function validate(rawConfig) {
    ...
    const config = stripUnderscores(rawConfig);
    ...
    return { errors, warnings, nodeCount: (config.nodes || []).length, uniqueIds: seenIds.size };
  }

  return { validate, stripUnderscores };
}));
